// ==UserScript==
// @name         ScavengerMineHeadless-ItsDave_ADA_NEW — 10-Minute Heartbeat
// @namespace    ItsDave_ADA
// @version      2.6.0
// @description  Auto-starts and maintains ScavengerMine sessions for the Midnight Scavenger Hunt mining NIGHT. Handles 429 rate-limit errors.
...
(function () {
  ...
  const RATE_LIMIT_WAIT_MS = 2 * 60_000; // 2 min wait for 429 errors

  let lastReloadAt = 0;
  let detected429 = false;
  ...
  // Intercept console errors to detect 429 and 503 responses
  function setupErrorDetection() {
    ...
    console.error = function(...args) {
      ...
      if (/429|rate.?limit|too many requests/i.test(message)) {
        if (!detected429) {
          detected429 = true;
          console.log(`${TAG} 🚨 429 rate-limit detected! Waiting 2 minutes before reload...`);
          handleRateLimitError();
        }
      } else if (/503|service.?unavailable|server.?error/i.test(message)) {
        if (!detected503) {
          detected503 = true;
          console.log(`${TAG} 🚨 503 service unavailable detected! Waiting 2 minutes before reload...`);
          handleRateLimitError();
        }
      }
    ...
    // Also intercept fetch to catch 429 and 503 at the network level
    const originalFetch = window.fetch;
    window.fetch = async function(...args) {
      try {
        const response = await originalFetch.apply(this, args);
        if (response.status === 429 && !detected429) {
          detected429 = true;
          console.log(`${TAG} 🚨 429 rate-limit detected! Waiting 2 minutes before reload...`);
          handleRateLimitError();
        } else if (response.status === 503 && !detected503) {
          detected503 = true;
          console.log(`${TAG} 🚨 503 service unavailable detected! Waiting 2 minutes before reload...`);
          handleRateLimitError();
        }
  ...
  function handleRateLimitError() {
    // Clear the heartbeat interval to stop normal operation
    if (heartbeatIntervalId) {
      clearInterval(heartbeatIntervalId);
      heartbeatIntervalId = null;
    }

    console.log(`${TAG} Pausing all activity for 2 minutes...`);

    setTimeout(() => {
      console.log(`${TAG} 2-minute wait complete. Reloading page...`);
      location.reload();
    }, RATE_LIMIT_WAIT_MS);
  }
  ...
  async function heartbeat() {
    if (detected429 || detected503) {
      console.log(`${TAG} Skipping heartbeat — handling rate limit or service error.`);
      return;
    }
  ...
  // Set up error detection for 429 and 503 responses
  setupErrorDetection();

  // Scroll to position 375 on page load
  function scrollToPosition() {
    const scrollContainer = document.querySelector('main div.w-full.flex-1.p-6.overflow-y-auto.p-6.px-6.py-6');
    if (scrollContainer) {
      scrollContainer.scrollTo(null, 375);
      console.log(`${TAG} Scrolled to position 375.`);
    } else {
      console.log(`${TAG} Scroll container not found.`);
    }
  }
  ...
  setTimeout(() => {
    scrollToPosition();
    heartbeat();
    heartbeatIntervalId = setInterval(heartbeat, HEARTBEAT_MS);
  }, BOOT_GRACE_MS);
})();
